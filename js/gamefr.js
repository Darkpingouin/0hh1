...
var Game = new (function() {
  var self = this,
      ...
      currentPuzzle = null,
      checkTOH = 0,
      ojoos = ['Magnifique','Spectaculaire','Impressionant','Remarquable','Wow','Incroyable','Bien joué','Fabuleux','Fantastique','Excellent','Super','Brillant','Splendide','Exceptionel'],
      remainingOjoos = [],
      endGameTOH1,
      endGameTOH2,
      endGameTOH3,
  ...
  function showSizes() {
    onHomeScreen = false;
    showGame();
    $('#boardsize').html('<span>Choisissez une taille</span>');
  ...
  function undo() {
    if (!undoStack.length) {
      ...
      if (!undone)
        grid.hint.show('Ceci est le bouton annuler.');
      else
        grid.hint.show('Il n\'y a rien à annuler.');
      return;
    }
    ...
    tile.mark();
    var s = 'Cette tuile a été inversé en ';
    if (value == 1) s += 'rouge.';
    if (value == 2) s += 'bleue.';
    if (value == 0) s += 'en un etat vide.'
    grid.hint.show(s);
    undone = true;
    clearTimeout(checkTOH);
    checkTOH = setTimeout(function(){checkForLevelComplete();}, 700);
  ...
  function updateTweetUrl(size) {
    var msg = '#0hh1 Je viens de finir un puzzle de ' + size + ' x ' + size + ' et mon score est de ' + getScore() + '. http://0hh1.com (or get the App!) ',
